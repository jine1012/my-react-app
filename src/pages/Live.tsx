...
export default function Live() {
  ...
  const [overrideSafetyState, setOverrideSafetyState] = useState<SafetyState | null>(null); // 🔥 추가
  ...
  const toggleSleepMode = () => {
    const newSleepMode = !sleepModeEnabled;
    setSleepModeEnabled(newSleepMode);
    
    // 🔥 수면 모드 켜질 때 바로 Safe 설정
    if (newSleepMode) {
      setCameraActive(true);
      setOverrideSafetyState('SAFE'); // 바로 Safe로 설정
    } else {
      setOverrideSafetyState(null); // 끌 때는 원래 상태로 복원
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50/30 via-indigo-50/20 to-purple-50/30">
      <div className="space-y-6 pb-20 px-4 pt-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 mt-1">수면 감지가 되면 모드가 켜져요</p>
              </div>
            </div>
            <button
            ...
            >
              {sleepModeEnabled ? "수면 모드 끄기" : "수면 모드 켜기"}
            </button>
          </div>
          ...
          {sleepModeEnabled && (
            <div className="mt-3 p-3 bg-green-50 rounded-xl border border-green-200">
              <div className="flex items-center gap-2 text-green-700">
                ...
                <span className="text-sm font-medium">수면 모드 ON - 아기가 자고 있어요</span>
              </div>
            </div>
          )}
        </div>
        ...
        {/* 🔥 질식감지 상태 표시 - 수정됨 */}
        <SafetyStatus
          safetyState={overrideSafetyState || toSafetyState(systemStatus?.currentState?.safety_state)}
        />
...
//         setCameraActive(true);
//         setOverrideSafetyState('SAFE'); // 질식 감지 기본값 Safe로 설정
        
//         // 카메라도 자동으로 켜기
//         await jetsonService.startCamera();
//       } else {
//         setOverrideSafetyState(null); // 감지 끌 때는 원래 상태로 복원
//       }
//     } catch (error) {
...
//                   </span>
//                 </span>
//                 <p className="text-xs text-slate-600 mt-1">AI가 자동으로 수면을 감지해서 모드를 전환해요</p>
//               </div>
//             </div>
//             <button
...
//               }`}
//             >
//               {sleepDetectionActive ? "끄기" : "켜기"}
//             </button>
//           </div>

//           {/* 🔥 수면 감지 활성화 상태 안내 추가 */}
//           {sleepDetectionActive && sleepModeEnabled && (
//             <div className="mt-3 p-3 bg-green-50 rounded-xl border border-green-200">
//               <div className="flex items-center gap-2 text-green-700">
//                 <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
//                 <span className="text-sm font-medium">수면 모드 ON - 아기가 자고 있어요</span>
